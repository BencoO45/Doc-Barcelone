...
Créer dans le répertoire /var/named/ les zones adéquates à l’aide d’un fichier db.xxxxxx (ou xxxxxx correspond à votre nom de domaine) par exemple, db.tours.btsssio.fr.
Initialiser la ou les zones dans /etc/bind/named.conf.local
Ajouter ou gérer les options globales du daemon Bind 9 dans /etc/bind/named.conf.options
Faire particulièrement attention à la syntaxe.
Penser aux commandes de test et redémarrer ou recharger le service.
